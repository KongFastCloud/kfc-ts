# Verification: Codemogger Integration & Repo File-Read Primitives for Seer
...
The codemogger integration and repo file-read primitives for seer have been correctly implemented. All acceptance criteria are met, all tests pass, and the design follows the single-process seer runtime model without introducing a separate indexing service.

## Acceptance Criteria Verification

### ✅ Codemogger-backed search is available to seer as a grounding primitive
- **File:** `packages/mastra/src/mcp/codemogger.ts` — defines a reusable `MCPServerRegistration` for codemogger
- **File:** `apps/seer/src/mcp.ts` — `createCodemoggerClient()` factory builds an MCP client from the registration
- **File:** `apps/seer/src/runtime.ts` — codemogger tools loaded at startup via `loadMCPTools("Codemogger", createCodemoggerClient)` and merged into the agent's tool set
...
- **File:** `apps/seer/src/tools/read-file.ts` — `readFileTool` created via `@mastra/core/tools`
- Tool ID: `read_file`
- Supports full file reads and line-range reads (`startLine`, `endLine`)
- Security: path traversal rejection, scoped to `SEER_REPO_ROOT`, 256 KB size cap, 500 line range cap
- Returns content with line numbers for agent-friendly formatting

### ✅ The grounding primitives fit inside the existing seer runtime model
- Both tools are bound in `runtime.ts` within the `SeerAgentLayer` Effect layer
...
| `apps/seer/src/tools/read-file.ts` | File-read grounding tool |
| `apps/seer/src/tools/read-file.test.ts` | File-read tool tests |
| `apps/seer/src/tools/index.ts` | Native tool exports |
| `apps/seer/src/mcp.ts` | MCP client factories (codemogger + glitchtip) |
| `apps/seer/src/runtime.ts` | Effect layer binding all tools to agent |
| `apps/seer/src/agent.ts` | Agent with grounding instructions in system prompt |
