 ...
 *
 * This suite proves that the TUI worker correctly wires the shared watchWorkflow
 * through the poll loop. It does NOT re-prove lifecycle internals (metadata
 * timing, prompt building, default failure reasons, exhausted-failure semantics)
 * — those contracts are owned by watchWorkflow.test.ts.
 *
 * Owned contracts:
 *  1. Sequential execution — tasks are processed one at a time, in order
 *  2. No-concurrency guarantee — only one task runs at any moment
 *  3. Callback behavior — onTaskComplete fires for success and failure
 *  4. Claim contention handling — lost claims skip without side effects
 *  5. Poll-loop correctness — empty queue, no fallback queries, stale recovery ordering
 *  6. Failure-then-recovery — worker continues accepting work after a failure
 *  7. Re-run delegation — worker does not internally track "already done"
 */
...
describe("worker orchestration: task routing through workflow", () => {
  test("successful task is routed through workflow and worker transitions idle → running → idle", async () => {
    ...
    // Worker wired the workflow: claim and close reached the right task
  ...
  test("failed task is routed to exhausted-failure path and worker returns to idle", async () => {
    ...
    // Worker routed to the failure path for the correct task
    const exhaustedCall = calls.find((c) => c.op === "markTaskExhaustedFailure")
    expect(exhaustedCall?.id).toBe("task-2")

    // Worker-specific: returns to idle after failure
    expect(states[states.length - 1]?.state).toBe("idle")
  })
})
...
describe("worker orchestration: single-worker no-concurrency guarantee", () => {
...
describe("worker orchestration: poll-loop correctness", () => {
  ...
  test("failed task is not re-polled if Beads excludes it from ready", async () => {
    ...
    // Worker processed the failure exactly once — no re-claim
    const claimCalls = calls.filter((c) => c.op === "claimTask" && c.id === "fail-1")
    expect(claimCalls.length).toBe(1)
...
describe("worker orchestration: callback behavior", () => {
