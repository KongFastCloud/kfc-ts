/**
 * ABOUTME: Logger configuration for ralphly.
 * Provides a stderr logfmt logger for CLI output and a JSON file logger
 * for persistent log history under .ralphly/logs/.
 */
...
const LOG_DIR = ".ralphly/logs"

const logFileName = (date: Date): string => {
  const yyyy = date.getFullYear()
  const mm = String(date.getMonth() + 1).padStart(2, "0")
  const dd = String(date.getDate()).padStart(2, "0")
  return `ralphly-${yyyy}-${mm}-${dd}.log`
}
